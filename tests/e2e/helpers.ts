...
export async function addTask(
  ...
  const titleInput = page
    .locator('.task-form__title-input, .add-task-form input[type="text"]')
    .first();
  ...
  if (description) {
    const descriptionInput = page
      .locator(".task-form__textarea, .add-task-form textarea")
      .first();
    await descriptionInput.fill(description);
  }

  await page
    .locator('.task-form__add-btn, .add-task-form button[type="submit"]')
    .first()
    .click({ force: true });
...
export async function editTask(
  ...
  await page
    .locator(".task-item--editing .task-edit-save-btn")
    .click({ force: true });
}
...
export async function clearCompletedTasks(page: Page): Promise<void> {
  await page
    .locator('.center-action-bar button[data-action="clear"]')
    .click({ force: true });
  await page
    .locator(".confirm-modal-backdrop, .modal.active")
    .first()
...
export async function deleteAllTasks(page: Page): Promise<void> {
  await page
    .locator('.center-action-bar button[data-action="delete"]')
    .click({ force: true });
  await page
    .locator(".confirm-modal-backdrop, .modal.active")
    .first()
...
export async function openSettings(page: Page): Promise<void> {
  await page
    .locator('.center-action-bar button[data-action="settings"]')
    .click({ force: true });
...
export async function waitForKarmaChange(
  page: Page,
  initialValue: number
): Promise<number> {
