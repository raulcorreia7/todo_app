...
function MusicPlayer(props: MusicPlayerProps) {
  ...
  const [currentTrack, setCurrentTrack] = createSignal(musicService.getCurrentTrack());
    ...
    <Show when={props.isOpen}>
      ...
      <div class="music-player music-player--open">
        <div class="music-player__header">
          ...
          <button class="music-player__close" onClick={props.onClose} aria-label="Close music player">
        ...
        <div class="music-player__body">
          ...
          <div class="music-player__track-list">
            <For each={musicService.getAllTracks()}>{(track: Track, index) => (
              <button 
                class={`music-player__track-item ${currentTrack().id === track.id ? "music-player__track-item--active" : ""}`}
                onClick={() => handleTrackSelect(index())}
              >
                <span class="music-player__track-item-name">{track.name}</span>
              </button>
            )}</For>
          </div>
        </div>
