...
test.describe('Task Persistence', () => {
  ...
  test('should store tasks in localStorage', async ({ page }) => {
    ...
    expect(parsed.some((t: { title: string }) => t.title === 'LocalStorage Task')).toBe(true);
  });
...
test.describe('Gamification Persistence', () => {
  ...
  test('should persist karma points after page reload', async ({ page }) => {
    ...
    const karmaBefore = await page.locator('.stat-card.stat-karma .stat-value').textContent();
    
    await page.reload();
    
    const karmaAfter = await page.locator('.stat-card.stat-karma .stat-value').textContent();
