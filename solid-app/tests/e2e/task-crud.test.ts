...
test.describe('Task Filtering', () => {
  ...
  test('should filter to show only active tasks', async ({ page }) => {
    ...
    await expect(page.locator('.task-item', { hasText: 'Active Task 1' })).toBeVisible();
    await expect(page.locator('.task-item', { hasText: 'Active Task 2' })).toBeVisible();
  });
  ...
  test('should filter to show only completed tasks', async ({ page }) => {
    ...
    expect(await getTaskCount(page)).toBe(2);
    await expect(page.locator('.task-item', { hasText: 'Completed Task 1' })).toBeVisible();
    await expect(page.locator('.task-item', { hasText: 'Completed Task 2' })).toBeVisible();
  });
});
...
test.describe('Clear Completed Tasks', () => {
  ...
  test('should clear all completed tasks', async ({ page }) => {
    ...
    expect(await getTaskCount(page)).toBe(1);
    await expect(page.locator('.task-item', { hasText: 'Active Task' })).toBeVisible();
  });
