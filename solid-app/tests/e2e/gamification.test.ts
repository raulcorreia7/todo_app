...
test.describe('Achievements', () => {
  ...
  test('should unlock first task achievement when creating first task', async ({ page }) => {
  ...
  test('should unlock completion achievement when completing first task', async ({ page }) => {
  ...
  test('should show locked achievements with question mark', async ({ page }) => {
    await openAchievements(page);
    ...
    if (count > 0) {
      ...
      await expect(firstLocked.locator('.achievement-card__icon-locked')).toHaveText('?');
    }
  });

  test('should display achievement notification when unlocked', async ({ page }) => {
