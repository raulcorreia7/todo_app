...
test.describe('Smoke Tests', () => {
  ...
  test('should display filter buttons', async ({ page }) => {
    ...
    await expect(filters.locator('button', { hasText: 'Active' })).toBeVisible();
    await expect(filters.locator('button', { hasText: 'Completed' })).toBeVisible();
  });
  ...
  test('should have settings button in action bar', async ({ page }) => {
    await expect(page.locator('.center-action-bar button[data-action="settings"]')).toBeVisible();
  });
});
...
test.describe('Critical User Flows', () => {
  ...
  test('should filter tasks correctly', async ({ page }) => {
    ...
    await page.locator('.task-item', { hasText: 'Done' }).locator('input[type="checkbox"]').click();
    
    await page.locator('.filter-group button', { hasText: 'Active' }).click();
    expect(await getTaskCount(page)).toBe(1);
    
    await page.locator('.filter-group button', { hasText: 'Completed' }).click();
    expect(await getTaskCount(page)).toBe(1);
...
test.describe('Accessibility', () => {
  ...
  test('should have aria labels on action buttons', async ({ page }) => {
    const settingsBtn = page.locator('.center-action-bar button[data-action="settings"]');
