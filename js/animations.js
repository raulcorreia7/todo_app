...
class AnimationManager {
        ...
        const animate = () => {
            ...
            particles.forEach(particle => {
                ...
                if (particle.divineMotion) {
                    ...
                    const divineX = Math.cos(particle.divineMotion.phase) * particle.divineMotion.amplitude;
                    const divineY = Math.sin(particle.divineMotion.phase) * particle.divineMotion.amplitude * 0.5;
                    
                    particle.x += divineX * 0.01 + particle.vx + particle.divineMotion.verticalDrift;
                ...
                if (type === 'loading') {
                    ...
                    particle.x = centerX + Math.cos(angle + 0.02) * Math.abs(particle.x - centerX);
                    particle.y = centerY + Math.sin(angle + 0.02) * Math.abs(particle.y - centerY);
                }
    ...
    updateCanvasForTheme(theme) {
        ...
        switch (theme.name) {
            case 'Arctic Sky':
                this.canvas.style.filter += ' drop-shadow(0 0 10px rgba(77, 166, 255, 0.3))';
                break;
            case 'Aurora':
                this.canvas.style.filter += ' drop-shadow(0 0 10px rgba(0, 255, 136, 0.3))';
                break;
            case 'Midnight':
                this.canvas.style.filter += ' drop-shadow(0 0 10px rgba(233, 69, 96, 0.3))';
                break;
        }
    }
    ...
    drawDivineParticle(particle) {
        ...
        const layers = [
            { size: particle.size * 4, alpha: 0.1, color: particle.secondaryGlowColor },
            { size: particle.size * 3, alpha: 0.2, color: particle.secondaryGlowColor },
        ...
        layers.forEach(layer => {
            ...
            gradient.addColorStop(0, layer.color + Math.floor(layer.alpha * 255).toString(16).padStart(2, '0'));
            gradient.addColorStop(0.5, layer.color + Math.floor(layer.alpha * 0.5 * 255).toString(16).padStart(2, '0'));
            gradient.addColorStop(1, 'transparent');
    ...
    showVignette() {
        ...
        if (theme) {
            ...
            const secondaryColor = this.hexToRgba(theme.glowSecondary || '#ffffff', 0.2);
