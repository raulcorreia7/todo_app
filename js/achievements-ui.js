...
class AchievementsUI {
  ...
  createAchievementsSection() {
    ...
    const soundSection = settingsContent.querySelector(
      '.settings-section[data-section="sound"], .settings-section.sound, .settings-section:nth-child(4)'
    );
    if (soundSection) {
  ...
  calculateProgress(achievement, stats) {
    ...
    if (
      achievement.id === "mindful_beginning" ||
      achievement.id === "peaceful_presence"
    ) {
  ...
  renderAchievement(achievement) {
    ...
    let lastUnlocked = "";
    try {
      const unlockedEntry = (gamificationManager.achievements || []).find(
  ...
  updateAchievements() {
    ...
    try {
      ...
      const idsMatch =
        cards.length === next.length &&
        cards.every(
          (c, i) => c.getAttribute("data-achievement") === next[i].id
        );
      ...
      if (idsMatch) {
        ...
        cards.forEach((card, i) => {
          ...
          card.style.transition =
            "opacity 160ms var(--ease-smooth, ease), transform 160ms var(--ease-smooth, ease)";
        });
    ...
    if (typeof gamificationManager !== "undefined") {
      ...
      if (grid) {
        ...
        const card =
          ...
          Array.from(grid.querySelectorAll(".achievement-card.unlocked")).find(
            (c) => {
              const title = c.querySelector("h5");
              return (
                title &&
                title.textContent &&
                title.textContent.trim() === achievementName
              );
            }
          );
        if (card) {
  ...
  bindHoverPopovers() {
    try {
      ...
      const show = (card) => {
        ...
        if (data.name)
          lines.push(
            `<div class="achv-popover-title">${this.escapeHtml(data.name)}</div>`
          );
        if (data.condition)
          lines.push(
            `<div class="achv-popover-cond">${this.escapeHtml(data.condition)}</div>`
          );
        if (data.lastUnlocked)
          lines.push(
            `<div class="achv-popover-meta">Unlocked: ${this.escapeHtml(data.lastUnlocked)}</div>`
          );
      ...
      grid.addEventListener("mouseover", this._achvHoverOver, {
        passive: true,
      });
