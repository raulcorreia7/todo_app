...
class GamificationManager {
  ...
  addTask() {
    ...
    if (!this.firstTaskCreated) {
      ...
      this.unlockAchievement(
        AchievementDefinitions.getAchievementById("first_task_created")
      );
    }
  ...
  editTask() {
    ...
    if (!this.firstTaskEdited) {
      ...
      this.unlockAchievement(
        AchievementDefinitions.getAchievementById("first_task_edited")
      );
    }
  ...
  deleteTask() {
    ...
    if (!this.firstTaskDeleted) {
      ...
      this.unlockAchievement(
        AchievementDefinitions.getAchievementById("first_task_deleted")
      );
    }
  ...
  unlockAchievement(achievement) {
    ...
    if (
      typeof achievementsUI !== "undefined" &&
      achievementsUI &&
      typeof achievementsUI.updateAchievements === "function"
    ) {
      ...
      requestAnimationFrame(() => {
        ...
        if (!grid) return;
        const byId = grid.querySelector(
          `.achievement-card.unlocked[data-achievement="${achievement.id}"]`
        );
        const card =
          byId ||
          Array.from(grid.querySelectorAll(".achievement-card.unlocked")).find(
            (c) => {
              ...
              return (
                title &&
                title.textContent &&
                title.textContent.trim() === achievement.name
              );
            }
          );
        if (card) {
          ...
          try {
            ...
            const isMilestone =
              milestone === 5 || milestone === 10 || milestone % 10 === 0;
      ...
      try {
        ...
        if (
          chimeEnabled &&
          (!this._lastChimeAt || now - this._lastChimeAt > 2500)
        ) {
          const soundAllowed =
            typeof settingsManager === "undefined"
              ? true
          ...
          const notMuted =
            typeof audioManager === "undefined"
              ? true
              : !(audioManager.getGlobalMute && audioManager.getGlobalMute());
          if (
            soundAllowed &&
            notMuted &&
            typeof audioManager !== "undefined" &&
  ...
  renderAchievementIcon(iconName) {
    try {
      ...
      if (
        ...
        typeof lucide.createIcon === "function"
      ) {
  ...
  spawnConfettiLite(container, options = {}) {
    try {
      ...
      const origin = this._findIconCenter(container) || {
        x: contRect.width / 2,
        y: contRect.height / 2,
      };
  ...
  _findIconCenter(container) {
    try {
      const icon = container.querySelector(
        ".achievement-icon, .zen-achievement-icon"
      );
      if (!icon) return null;
