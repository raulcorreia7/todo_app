(function(global){
  ...
  App.TooltipService = (function() {
    ...
    function show(target, text) {
      ...
      var desiredTop = desktop ? (rect.top - ttRect.height - 10) : (rect.bottom + 10);
      var desiredLeft = rect.left + rect.width / 2 - ttRect.width / 2;

      // Constrain to viewport
      var pad = 8;
      var left = Math.max(pad, Math.min(desiredLeft, window.innerWidth - ttRect.width - pad));
      var top = Math.max(pad, Math.min(desiredTop, window.innerHeight - ttRect.height - pad));
    ...
    function bindAll(root) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        (function(el){
          ...
          el.addEventListener('focus', function(){ show(el); }, true);
          el.addEventListener('blur', hide, true);
          // Touch support (brief preview)
          el.addEventListener('touchstart', function(){ show(el); }, {passive:true});
